...
func wireApp(confServer *conf.Server, confData *conf.Data, logger log.Logger) (*kratos.App, func(), error) {
	...
	imageUsecase := biz.NewFileUsecase(confData, logger)
	imageService := service.NewFileService(imageUsecase, confData, logger)
	...
	difyRepo := data.NewDifyRepo(dataData, logger)
	difyUsecase := biz.NewDifyUsecase(difyRepo, logger)
	difyService := service.NewDifyService(difyUsecase, logger)
	httpServer := server.NewHTTPServer(confServer, confData, roleTemplateService, meetingTemplateService, imageService, meetingService, difyService, logger)
